  ...
  */
  ...
  var QDate = Vue.extend({
    ...
    computed: {
      ...
      hasRangeStart: function hasRangeStart () {
        return typeof this.rangeStart === 'string' && this.rangeStart.length > 6
      },

      rangeStartEnd: function rangeStartEnd () {
        if (
          this.hasRangeStart === true &&
          typeof this.rangeEnd === 'string' && this.rangeEnd.length > 6
        ) {
          return [this.rangeStart, this.rangeEnd].sort()
        }
      },
      ...
      rangeCheckFn: function rangeCheckFn () {
        ...
        if (Array.isArray(this.ranges) === true) {
          var min = this.innerModel.year + '/' + pad(this.innerModel.month) + '/01';
          var max = this.innerModel.year + '/' + pad(this.innerModel.month) + '/99';
          var intervalRanges = [];
          var simpleRanges = this.ranges.filter(function (range) {
            if (Array.isArray(range)) {
              range = range.slice(0, 2).sort();
              if (range.length > 1 && min <= range[1] && max >= range[0]) {
          ...
          return function (date) {
            if (
              this$1.rangeStartEnd !== void 0 &&
              this$1.rangeStartEnd[0] <= date &&
              date <= this$1.rangeStartEnd[1]
            ) {
              return {
                outline: true,
                start: this$1.rangeStartEnd[0] === date,
                end: this$1.rangeStartEnd[1] === date,
            ...
            for (var i = 0; i < intervalsLength; i++) {
              ...
              if (interval[0] <= date && date <= interval[1]) {
                return {
      ...
      days: function days () {
        ...
        for (var i$1 = 1; i$1 <= this.daysInMonth; i$1++) {
          ...
          if (this.options !== void 0 && this.isSelectable(day) !== true) {
            ...
            var rangeFlags = this.rangeCheckFn === void 0 ? void 0 : this.rangeCheckFn(prefix + pad(i$1));
            var range = getRangeClass(rangeFlags);

            var flat = rangeFlags === void 0 ||
              rangeFlags.outline === true ||
              (rangeFlags.start !== true && rangeFlags.end !== true);

            var config = { i: i$1, in: true, flat: flat, event: event, range: range };
        ...
        if (left > 0) {
          ...
          for (var i$3 = 1; i$3 <= afterDays; i$3++) {
            res.push({ i: i$3, fill: true, range: '' });
          }
        }
    ...
    methods: {
                  ...
                  day.in === true
                    ? h(QBtn, {
                      ...
                      on: cache(this$1, 'day#' + day.i, {
                        ...
                        mouseenter: function () { this$1.__rangeEndHover(day.i); }
                      })
                    }, day.event !== false ? [
      ...
      __rangeEndHover: function __rangeEndHover (day) {
        if (this.hasRangeStart === true) {
          var rangeVal = this.innerModel.year + '/' + pad(this.innerModel.month) + '/' + pad(day);
          this.$emit('extend:range', [ this.rangeStart, rangeVal ]);
        }
      },
      ...
      __updateValue: function __updateValue (date, reason) {
        ...
        if (reason === 'day' && this.rangeCheckFn !== void 0) {
          var rangeVal = date.year + '/' + pad(date.month) + '/' + pad(date.day);
          if (this.hasRangeStart === true) {
            this.$emit('update:range', [ this.rangeStart, rangeVal ].sort());
          }
          else {
            this.$emit('start:range', rangeVal);
          }
        }
