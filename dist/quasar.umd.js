/*!
 * Quasar Framework v1.0.5-test.20
...
(function (global, factory) {
  ...
  */
  ...
  function sumSize (sizeAgg, size, from, to) {
    ...
    var total = sizeAgg.slice(fromAgg, toAgg).reduce(function (acc, h) { return acc + h; }, 0);

    if (from % aggBucketSize !== 0) {
      total -= size.slice(fromAgg * aggBucketSize, from).reduce(function (acc, h) { return acc + h; }, 0);
    }
    if (to % aggBucketSize !== 0 && to !== lastTo) {
      total -= size.slice(to, toAgg * aggBucketSize).reduce(function (acc, h) { return acc + h; }, 0);
    }
