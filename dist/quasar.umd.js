/*!
 * Quasar Framework v1.0.5-test.10
...
(function (global, factory) {
  ...
  function getAllChildren (vm, children) {
    ...
    vm.$children.forEach(getChildFn(children));
    return children
  }
      ...
      var ctx = {
        ...
        keyboardStart: function keyboardStart (evt) {
          if (ctx.eventStarter === void 0 && keyboard.includes(evt.keyCode)) {
            el.focus();
            el.addEventListener('blur', ctx.keyboardEnd, passiveCapture$1);
            document.addEventListener('keyup', ctx.keyboardEnd, passiveCapture$1);
            ctx.start(evt, false, true);
          }
        },

        keyboardEnd: function keyboardEnd (evt) {
          el.removeEventListener('blur', ctx.keyboardEnd, passiveCapture$1);
          document.removeEventListener('keyup', ctx.keyboardEnd, passiveCapture$1);
          ctx.end(evt);
        },
