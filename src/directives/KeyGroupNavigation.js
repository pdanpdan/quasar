import { stop, prevent, addEvt, cleanEvt, listenOpts } from '../utils/event.js'
import {
  FOCUSABLE_SELECTOR,
  KEY_SKIP_SELECTOR,
...
let inPointerInteraction = false
let interactionHandlerCounter = 0
const { passiveCapture } = listenOpts
const interactionEvents = {
  on: ['mousedown', 'touchstart'],
  off: ['mouseup', 'touchend', 'touchcancel', 'dragstart']
}

function interactionHandlerOn () {
  inPointerInteraction !== true && (inPointerInteraction = true)
}

function interactionHandlerOff () {
  inPointerInteraction !== false && (inPointerInteraction = false)
}

function setInteractionHandler (state) {
  interactionHandlerCounter = Math.max(0, interactionHandlerCounter + (state === true ? 1 : 0))

  if (interactionHandlerCounter === 1) {
    interactionEvents.on.forEach(event => {
      document.documentElement.addEventListener(event, interactionHandlerOn, passiveCapture)
    })
    interactionEvents.off.forEach(event => {
      document.documentElement.addEventListener(event, interactionHandlerOff, passiveCapture)
    })
  }
  else if (interactionHandlerCounter === 0) {
    interactionEvents.on.forEach(event => {
      document.documentElement.removeEventListener(event, interactionHandlerOn, passiveCapture)
    })
    interactionEvents.off.forEach(event => {
      document.documentElement.removeEventListener(event, interactionHandlerOff, passiveCapture)
    })
  }
}
...
export default {
  ...
  bind (el, binding) {
    const ctx = {
      ...
      focusIn (evt) {
        el.classList[inPointerInteraction === true ? 'remove' : 'add'](ctx.activeClass)
