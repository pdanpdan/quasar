  ...
  computed: {
    ...
    rangeCheckFn () {
      ...
      if (Array.isArray(this.ranges) === true) {
        const min = this.innerModel.year + '/' + pad(this.innerModel.month) + '/01'
        const max = this.innerModel.year + '/' + pad(this.innerModel.month) + '/99'
        ...
        return date => {
          if (
            this.rangeStartEnd !== void 0 &&
            this.rangeStartEnd[0] <= date &&
            date <= this.rangeStartEnd[1]
          ) {
            return {
              outline: true,
              start: this.rangeStartEnd[0] === date,
              end: this.rangeStartEnd[1] === date,
  ...
  methods: {
                ...
                day.in === true
                  ? h(QBtn, {
                    ...
                    on: cache(this, 'day#' + day.i, {
                      ...
                      mouseenter: () => { this.__rangeEndHover(day.i) }
                    })
                  }, day.event !== false ? [
    ...
    __rangeEndHover (day) {
      if (this.hasRangeStart === true) {
        const rangeVal = this.innerModel.year + '/' + pad(this.innerModel.month) + '/' + pad(day)
    ...
    __updateValue (date, reason) {
      ...
      if (reason === 'day' && this.rangeCheckFn !== void 0) {
        const rangeVal = date.year + '/' + pad(date.month) + '/' + pad(date.day)
        if (this.hasRangeStart === true) {
