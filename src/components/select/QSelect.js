  ...
  methods: {
    ...
    __hydrateOptions (ev, toIndex) {
      ...
      if (toIndex === void 0 || content === null) {
        ...
        toIndex = -1
        for (let i = Math.round(scrollTop + viewHeight / 2); i >= 0 && toIndex < toIndexMax;) {
          toIndex++
    ...
    __setOptionsSliceRange (toIndex, target, scroll) {
      const
      ...
      if (from !== this.optionsSliceRange.from || to !== this.optionsSliceRange.to) {
        ...
        requestAnimationFrame(() => {
          const content = target.querySelector('.q-select__options--content')
        ...
        requestAnimationFrame(() => {
          if (target !== void 0) {
            target.scrollTop = Math.max(0, Math.round(
              this.optionsHeights.slice(0, toIndex).reduce((acc, h) => acc + h, 0) +
              this.optionsHeights[toIndex] / 2 -
