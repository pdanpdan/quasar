...
function debounce (fn) {
  let timeoutWait, timeoutTrigger
  ...
  function debounced (/* ...args */) {
    ...
    const later = () => {
      ...
      timeoutWait = void 0
      timeoutTrigger = void 0
      fn.apply(this, args)
    }

    if (timeoutWait === void 0) {
      timeoutTrigger = setTimeout(later, 300)
    }
    else {
      clearTimeout(timeoutWait)
    }
    timeoutWait = setTimeout(later, 150)
  }
...
export default {
  ...
  methods: {
    ...
    __setVirtualScrollSliceRange (scrollEl, scrollDetails, toIndex, offset, align) {
      ...
      const scrollToEnd = this.prevScrollStart > scrollDetails.scrollStart
      ...
      requestAnimationFrame(() => {
        ...
        if (alignForce === true || scrollToEnd === true) {
          setScroll(
            scrollEl,
            scrollPosition,
            this.virtualScrollHorizontal,
  ...
  beforeMount () {
    this.__onVirtualScrollEvt = debounce(this.__onVirtualScrollEvt)
    this.__setVirtualScrollSize()
  },
